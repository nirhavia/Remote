...
public class MainActivity extends AppCompatActivity {
    ...
    @Override protected void onCreate(Bundle s) {
        ...
        client=new TvClient(this); tvStatus=findViewById(R.id.tvStatus); etIp=findViewById(R.id.etIp);
        String saved=client.getSavedIp(); if(!saved.isEmpty())etIp.setText(saved);
        client.setListener(new TvClient.Listener(){
            public void onConnected(){runOnUiThread(()->{tvStatus.setText("מחובר");tvStatus.setTextColor(0xFF4CAF50);});}
            public void onDisconnected(){runOnUiThread(()->{tvStatus.setText("מנותק");tvStatus.setTextColor(0xFFE94560);});}
            public void onError(String m){runOnUiThread(()->{tvStatus.setText("שגיאה");tvStatus.setTextColor(0xFFFF9800);});}
        });
        setup(); if(!saved.isEmpty())client.connect(saved);
    }
    private void setup(){
        findViewById(R.id.btnConnect).setOnClickListener(v->{
            ...
            client.saveIp(ip);tvStatus.setText("מתחבר...");tvStatus.setTextColor(0xFF8892A4);client.connect(ip);
        });
